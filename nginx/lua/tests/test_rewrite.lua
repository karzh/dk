...
function testUri()
	...
	local r = routes.route({ uri="/ganji/12345", host="bj.example.com"})
	assertEquals( r.group, "v1" )
	local r = routes.route({ uri="/ganji/12345abc", host="bj.example.com"})
	assertEquals( r, null)
	local r = routes.route({ uri="/abc/ganji2/12345", host="bj.example.com"})
	assertEquals( r.group, "verr"  )
	local r = routes.route({ uri="/ganji/12345/abc", host="bj.example.com"})
	assertEquals( r.group, "v2" )
	local r = routes.route({ uri="/", host="bj.example.com"})
...
function testHost()
	...
	local r = routes.route({ host="bj.example.com"})
	assertEquals( r.group, "ganji_com" )
	local r = routes.route({ host="bj.example.com.cn"})
...
function testNested()
	...
	local r = routes.route({ uri="/ganji1", host="bj.example.com", method="POST"})
	assertEquals( r.group, "ganji1.post" )
	local r = routes.route({ uri="/ganji1", host="bj.example.com", method="GET"})
...
function testCity()
	...
	local r = routes.route({ host="hk.example.com"}) -- not in test data
	assertEquals( r, nil)
	local r = routes.route({ host="123.example.com"})
	assertEquals( r, nil)
end
...
function testCity2()
	...
	assertEquals( r.city, 1)
	local r = routes.route({ host="sh.example.com"}) -- not in test data
...
function testCombo()
	...
	local r = routes.route({ uri="/linlin", host="bj.example.com"} )
	assertEquals( r.group, "ganji_city" )
	local r = routes.route({ uri="/jiajia",host="sz.example.com"} ) -- not in test data
	assertEquals( r, nil )
	local r = routes.route({ uri="/jiajia/hello",host="bj.example.com"} ) -- not in test data
	assertEquals( r.group, "ganji_city" )
end
